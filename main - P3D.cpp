...
// ray:
// depth: numero maximo de interacções?
// float: Indice de Refracção?
Color rayTracing( Ray ray, int depth, float RefrIndex)
{
	//INSERT HERE YOUR CODE TODO

	//Detectar se interseta com algum de todos os objectos
		//temos de retornar a intersecção mais próxima e o objecto intercetado

	//Caso nao haja intersection
	//if (!GetIntersection(Ray ray, vector* posIntersection)) return BACKGROUND; //se nao interceptar nenhum objecto entao, retorna a cor do Background


	//Caso haja intersection
		/*else {
			color = object material's ambient color;
			normal = GetNormal;													//compute normal at the hit point
			for (each source light) {
				L = unit light vector from hit point to light source
					if (L.normal>0)
						if (!point in shadow);									//trace shadow ray
							color += difuse color + specular color
			}

			//caso tenhamos atingido o maximo de Depth
			if (depth==0) return color;


			//Calculate the Reflexion Ray
			if (reflective object) {
				rRay = calculate ray in the reflected direction;
				rColor = trace([scene, point, rRay direction], depth - 1,float RefrIndex);
			...
			if (translucid object) {
				...
				tColor = trace([scene, point, tRay direction], depth - 1,float RefrIndex);
				//reduce tColor by the transmittance coeefficient and add to color;
			}
		}
...
{
    // INSERT HERE YOUR CODE FOR PARSING NFF FILES  TODO
